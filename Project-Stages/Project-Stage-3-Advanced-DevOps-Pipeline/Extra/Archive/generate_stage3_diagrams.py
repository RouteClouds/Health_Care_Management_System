...
from diagrams.onprem.analytics import Elasticsearch, Kibana
...
from diagrams.onprem.monitoring import Jaeger
...
from diagrams.onprem.analytics import SonarQube
...
def generate_cicd_pipeline_diagram():
    ...
    with Diagram("Stage-3 CI/CD Pipeline", 
        ...
        with Cluster("Deployment"):
            ...
            kubectl_apply = Kubernetes("kubectl Apply")
        
        with Cluster("Environments"):
            dev_env = Kubernetes("Development")
            staging_env = Kubernetes("Staging")
            prod_env = Kubernetes("Production")
...
def generate_monitoring_observability_diagram():
    ...
    with Diagram("Stage-3 Monitoring & Observability", 
        ...
        with Cluster("Application Layer"):
            frontend = Kubernetes("Frontend App")
        ...
        with Cluster("Distributed Tracing"):
            ...
            jaeger_storage = Jaeger("Jaeger Storage")
            opentelemetry = Jaeger("OpenTelemetry\n(Instrumentation)")
        
        with Cluster("Logging Stack"):
            fluentd = Fluentd("Fluentd\n(Log Collector)")
            elasticsearch = Elasticsearch("Elasticsearch\n(Log Storage)")
            kibana = Kibana("Kibana\n(Log Analytics)")
            logstash = Fluentd("Logstash\n(Log Processing)")
    ...
    with Diagram("Stage-3 GitOps Workflow", 
        ...
        with Cluster("Kubernetes Clusters"):
            with Cluster("Development"):
                dev_cluster = Kubernetes("Dev Cluster")
                dev_apps = Kubernetes("Dev Applications")
            
            with Cluster("Staging"):
                staging_cluster = Kubernetes("Staging Cluster")
                staging_apps = Kubernetes("Staging Applications")
            
            with Cluster("Production"):
                prod_cluster = Kubernetes("Production Cluster")
                prod_apps = Kubernetes("Production Applications")
    ...
    with Diagram("Stage-3 Istio Service Mesh", 
        ...
        with Cluster("Application Services"):
            with Cluster("Frontend Service"):
                frontend = Kubernetes("Frontend Pod")
                frontend_proxy = Istio("Envoy Proxy")
            
            with Cluster("Backend Service"):
                backend = Kubernetes("Backend Pod")
...
def generate_security_architecture_diagram():
    ...
    with Diagram("Stage-3 Security Architecture", 
        ...
        with Cluster("Secrets Management"):
            ...
            k8s_secrets = Kubernetes("Kubernetes Secrets")
            external_secrets = Kubernetes("External Secrets Operator")
        
        with Cluster("Container Security"):
            trivy = Trivy("Trivy\n(Container Scanning)")
            falco = Server("Falco\n(Runtime Security)")
            opa_gatekeeper = Server("OPA Gatekeeper\n(Policy Enforcement)")
            pod_security = Kubernetes("Pod Security Standards")
    ...
    with Diagram("Stage-3 Performance & Scaling", 
        ...
        with Cluster("Auto Scaling"):
            with Cluster("Horizontal Pod Autoscaler (HPA)"):
                hpa_frontend = Kubernetes("Frontend HPA")
                hpa_backend = Kubernetes("Backend HPA")
                hpa_metrics = Prometheus("HPA Metrics")
            
            with Cluster("Vertical Pod Autoscaler (VPA)"):
                vpa_frontend = Kubernetes("Frontend VPA")
                vpa_backend = Kubernetes("Backend VPA")
            
            with Cluster("Cluster Autoscaler"):
                cluster_autoscaler = Kubernetes("Cluster Autoscaler")
                node_groups = EC2("Node Groups")
        
        with Cluster("Application Layer"):
            with Cluster("Frontend Service"):
                frontend_pod1 = Kubernetes("Frontend Pod 1")
                frontend_pod2 = Kubernetes("Frontend Pod 2")
                frontend_pod3 = Kubernetes("Frontend Pod 3")
            
            with Cluster("Backend Service"):
                backend_pod1 = Kubernetes("Backend Pod 1")
                backend_pod2 = Kubernetes("Backend Pod 2")
                backend_pod3 = Kubernetes("Backend Pod 3")
                backend_pod4 = Kubernetes("Backend Pod 4")
