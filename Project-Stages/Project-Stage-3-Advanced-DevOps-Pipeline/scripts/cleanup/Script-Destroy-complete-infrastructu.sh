...
# === Audit-driven deletion helpers ===
# Deletion report accumulators
REPORT_DELETED=()    # e.g., "vpc: vpc-123"
REPORT_FAILED=()     # e.g., "vpc: vpc-123 (reason)"
REPORT_NOT_FOUND=()  # e.g., "vpc: vpc-123"

# Parsed resources (globals)
AUDIT_FILE=""
PARSED_VPCS=()
PARSED_NAT_GWS=()
PARSED_EIPS=()
...
# Extract all content lines for a section (bounded until next blank line)
...
# Return only table rows (lines that start with '|' and not rulers '+')
extract_table_rows() { awk '/^\|/ && $0 !~ /^\+/' | sed '/^| *Describe/ d'; }

# Extract a 1-based pipe-delimited column
get_col() { awk -F '|' -v col="$1" '{if($0 ~ /^\|/){print $col}}' | trim; }

prompt_for_audit() {
  read -r -p "Have you run the audit script? (y/N): " RAN_AUDIT
  RAN_AUDIT="${RAN_AUDIT,,}"
  if [[ "$RAN_AUDIT" == "y" ]]; then
    read -r -p "Enter path to audit report (aws-resources-audit-YYYYMMDD-HHMMSS.txt): " AUDIT_FILE
    if [[ ! -f "$AUDIT_FILE" ]]; then
      log_error "Audit file not found: $AUDIT_FILE"; exit 1
    fi
    log_info "Using audit file: $AUDIT_FILE"
  else
    log_warning "Proceeding without audit file. Will use dynamic discovery fallbacks."
  fi
}

# Execute command with dry run support + reporting
execute_command() {
    local cmd="$1"
    local description="$2"
    local res_type="${3:-}"
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        [[ -n "$res_type" && -n "$res_id" ]] && REPORT_DELETED+=("$res_type: $res_id (dry-run)")
    else
        log_info "$description"
        if eval "$cmd" 2>/dev/null; then
            log_success "$description completed"
            [[ -n "$res_type" && -n "$res_id" ]] && REPORT_DELETED+=("$res_type: $res_id")
        else
            log_warning "$description failed (may not exist)"
            [[ -n "$res_type" && -n "$res_id" ]] && REPORT_FAILED+=("$res_type: $res_id (delete failed)")
        fi
    fi
}
...
# Verifiers
verify_vpc() { aws ec2 describe-vpcs --vpc-ids "$1" --region "$REGION" >/dev/null 2>&1; }
verify_nat() { aws ec2 describe-nat-gateways --nat-gateway-ids "$1" --region "$REGION" >/dev/null 2>&1; }
verify_eip() { aws ec2 describe-addresses --allocation-ids "$1" --region "$REGION" >/dev/null 2>&1; }
verify_igw() { aws ec2 describe-internet-gateways --internet-gateway-ids "$1" --region "$REGION" >/dev/null 2>&1; }
verify_lb_by_name() { aws elbv2 describe-load-balancers --names "$1" --region "$REGION" >/dev/null 2>&1; }
verify_cluster() { aws eks describe-cluster --name "$1" --region "$REGION" >/dev/null 2>&1; }
verify_rds() { aws rds describe-db-instances --db-instance-identifier "$1" --region "$REGION" >/dev/null 2>&1; }
verify_ecr() { aws ecr describe-repositories --repository-names "$1" --region "$REGION" >/dev/null 2>&1; }
verify_bucket() { aws s3api head-bucket --bucket "$1" >/dev/null 2>&1; }

parse_audit() {
  [[ -z "$AUDIT_FILE" ]] && return 0

  readarray -t PARSED_VPCS < <(
    extract_section_lines "VPCs" "$AUDIT_FILE" | extract_table_rows | awk -F '|' '/healthcare|stage3/ {print $5}' | trim
  )

  readarray -t PARSED_NAT_GWS < <(
    extract_section_lines "NAT Gateways" "$AUDIT_FILE" | extract_table_rows | get_col 2
  )

  readarray -t PARSED_EKS_CLUSTERS < <(
    extract_section_lines "EKS Clusters" "$AUDIT_FILE" | extract_table_rows | get_col 2
  )

  readarray -t PARSED_RDS_IDS < <(
    extract_section_lines "RDS Instances" "$AUDIT_FILE" | extract_table_rows | get_col 2
  )

  readarray -t PARSED_ECR_REPOS < <(
    extract_section_lines "ECR Repositories" "$AUDIT_FILE" | extract_table_rows | get_col 2
  )

  readarray -t PARSED_S3_BUCKETS < <(
    extract_section_lines "S3 Buckets" "$AUDIT_FILE" | extract_table_rows | get_col 2
  )

  readarray -t PARSED_EIPS < <(
    extract_section_lines "Elastic IPs" "$AUDIT_FILE" | extract_table_rows | get_col 2
  )

  readarray -t PARSED_LB_NAMES < <(
    extract_section_lines "Application/Network Load Balancers" "$AUDIT_FILE" | extract_table_rows | get_col 2
  )

  readarray -t PARSED_IGWS < <(
    extract_section_lines "Internet Gateways" "$AUDIT_FILE" | extract_table_rows | get_col 2
  )
}
  ...
  if [[ ${#PARSED_VPCS[@]} -eq 0 ]]; then
    readarray -t PARSED_VPCS < <(
      aws ec2 describe-vpcs --region "$REGION" \
        --filters "Name=tag:Name,Values=*healthcare*stage3*" \
        --query 'Vpcs[].VpcId' --output text 2>/dev/null | tr '\t' '\n' | trim
    )
  fi
  if [[ ${#PARSED_NAT_GWS[@]} -eq 0 && ${#PARSED_VPCS[@]} -gt 0 ]]; then
    readarray -t PARSED_NAT_GWS < <(
      aws ec2 describe-nat-gateways --region "$REGION" \
        --filter "Name=vpc-id,Values=${PARSED_VPCS[*]}" \
        --query 'NatGateways[].NatGatewayId' --output text 2>/dev/null | tr '\t' '\n' | trim
    )
  fi
  if [[ ${#PARSED_EKS_CLUSTERS[@]} -eq 0 ]]; then
    readarray -t PARSED_EKS_CLUSTERS < <(
      aws eks list-clusters --region "$REGION" --query 'clusters[]' --output text 2>/dev/null \
      | tr '\t' '\n' | grep -E 'healthcare|stage3' || true
    )
  fi
  if [[ ${#PARSED_RDS_IDS[@]} -eq 0 ]]; then
    readarray -t PARSED_RDS_IDS < <(
      aws rds describe-db-instances --region "$REGION" \
        --query 'DBInstances[?contains(DBInstanceIdentifier, `healthcare`) || contains(DBInstanceIdentifier, `stage3`)].DBInstanceIdentifier' \
        --output text 2>/dev/null | tr '\t' '\n' | trim
    )
  fi
  if [[ ${#PARSED_ECR_REPOS[@]} -eq 0 ]]; then
    readarray -t PARSED_ECR_REPOS < <(
      aws ecr describe-repositories --region "$REGION" \
        --query 'repositories[?contains(repositoryName, `healthcare`) || contains(repositoryName, `stage3`)].repositoryName' \
        --output text 2>/dev/null | tr '\t' '\n' | trim
    )
  fi
  if [[ ${#PARSED_S3_BUCKETS[@]} -eq 0 ]]; then
    readarray -t PARSED_S3_BUCKETS < <(
      aws s3api list-buckets --query 'Buckets[].Name' --output text 2>/dev/null \
      | tr '\t' '\n' | grep -E 'healthcare|stage3' || true
    )
  fi
  if [[ ${#PARSED_LB_NAMES[@]} -eq 0 ]]; then
    readarray -t PARSED_LB_NAMES < <(
      aws elbv2 describe-load-balancers --region "$REGION" \
        --query 'LoadBalancers[?contains(LoadBalancerName, `healthcare`) || contains(LoadBalancerName, `stage3`)].LoadBalancerName' \
        --output text 2>/dev/null | tr '\t' '\n' | trim
    )
  fi
}
...
cleanup_eks_cluster() {
    ...
    local cluster_name="${PARSED_EKS_CLUSTERS[0]:-healthcare-eks-stage3-dev}"
...
cleanup_nat_gateways_and_eips() {
    ...
    # Delete NAT Gateways from parsed audit (or discovered)
    if [[ ${#PARSED_NAT_GWS[@]} -eq 0 ]]; then
        log_info "No NAT Gateways parsed; skipping explicit NAT deletion phase (catch-all still runs by VPC)."
    else
        for nat_gw in "${PARSED_NAT_GWS[@]}"; do
            [[ -z "$nat_gw" ]] && continue
            if ! verify_nat "$nat_gw"; then
                REPORT_NOT_FOUND+=("nat-gateway: $nat_gw")
                continue
            fi
            execute_command "aws ec2 delete-nat-gateway --nat-gateway-id '$nat_gw' --region '$REGION'" \
                "Delete NAT Gateway $nat_gw" "nat-gateway" "$nat_gw"
        done
    ...
    # Release Elastic IPs from parsed audit
    if [[ ${#PARSED_EIPS[@]} -eq 0 ]]; then
        log_info "No Elastic IPs parsed; skipping EIP release phase."
    else
        for eip in "${PARSED_EIPS[@]}"; do
            [[ -z "$eip" ]] && continue
            if ! verify_eip "$eip"; then
                REPORT_NOT_FOUND+=("eip: $eip")
                continue
            fi
            execute_command "aws ec2 release-address --allocation-id '$eip' --region '$REGION'" \
                "Release Elastic IP $eip" "eip" "$eip"
        done
    fi
}
...
cleanup_vpcs_and_networking() {
    ...
    # VPCs from parsed audit (excluding default VPC)
    local vpcs=("${PARSED_VPCS[@]}")

    for vpc_id in "${vpcs[@]}"; do
        [[ -z "$vpc_id" ]] && continue
        if ! verify_vpc "$vpc_id"; then
            REPORT_NOT_FOUND+=("vpc: $vpc_id")
            continue
        fi
        ...
        if [[ -n "$security_groups" ]]; then
            for sg in $security_groups; do
                execute_command "aws ec2 delete-security-group --group-id '$sg' --region '$REGION'" \
                    "Delete security group $sg" "security-group" "$sg"
            done
        fi
        ...
        if [[ -n "$subnets" ]]; then
            for subnet in $subnets; do
                execute_command "aws ec2 delete-subnet --subnet-id '$subnet' --region '$REGION'" \
                    "Delete subnet $subnet" "subnet" "$subnet"
            done
        fi
        ...
        if [[ -n "$nacls" ]]; then
            for nacl in $nacls; do
                execute_command "aws ec2 delete-network-acl --network-acl-id '$nacl' --region '$REGION'" \
                    "Delete network ACL $nacl" "network-acl" "$nacl"
            done
        fi
        ...
        if [[ -n "$vpc_nats" ]]; then
            for nat in $vpc_nats; do
                if ! verify_nat "$nat"; then
                    REPORT_NOT_FOUND+=("nat-gateway: $nat")
                    continue
                fi
                execute_command "aws ec2 delete-nat-gateway --nat-gateway-id '$nat' --region '$REGION'" \
                    "Delete NAT Gateway $nat (VPC catch-all)" "nat-gateway" "$nat"
            done
        fi

        # Detach and delete internet gateways (only ones attached to this VPC)
        local igws
...
main() {
    ...
    # Prompt for audit and parse; discover missing
    prompt_for_audit
    parse_audit
    discover_if_missing

    log_info "Parsed counts — VPCs: ${#PARSED_VPCS[@]}, NAT GWs: ${#PARSED_NAT_GWS[@]}, EKS: ${#PARSED_EKS_CLUSTERS[@]}, RDS: ${#PARSED_RDS_IDS[@]}, ECR: ${#PARSED_ECR_REPOS[@]}, S3: ${#PARSED_S3_BUCKETS[@]}, LBs: ${#PARSED_LB_NAMES[@]}, EIPs: ${#PARSED_EIPS[@]}"
    ...
    # Final report
    echo ""
    log_phase "Final Deletion Report"
    ...
    echo ""
    echo "🕳️ Not found (already deleted or never existed):"
    if [[ ${#REPORT_NOT_FOUND[@]} -gt 0 ]]; then
      printf '  - %s\n' "${REPORT_NOT_FOUND[@]}"
    else
      echo "  (none)"
    fi
